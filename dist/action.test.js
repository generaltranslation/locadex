...
describe('GitHub Action', () => {
    ...
    it('should get required API key input', async () => {
        ...
        expect(mockCore.exportVariable).toHaveBeenCalledWith('ANTHROPIC_API_KEY', 'test-api-key');
    });
    ...
    it('should handle errors and set failed status', async () => {
        ...
        const { run } = await import('./action');
        await run();
        expect(mockCore.setFailed).toHaveBeenCalledWith('Action failed with error: Error: Command failed');
    });
});
